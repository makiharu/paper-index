...
export class ImageMagickImagePreprocessor implements ImagePreprocessor {
  constructor(private readonly workDirectory: string, private readonly command = "convert") {}
  ...
  async prepare(imagePath: string): Promise<PreparedImage> {
    ...
    const outputPath = path.join(this.workDirectory, `${path.basename(imagePath, path.extname(imagePath))}.ocr.jpg`);
    try {
      await execFileAsync(this.command, [imagePath, outputPath]);
    } catch (error) {
